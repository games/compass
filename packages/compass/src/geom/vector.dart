...
class Vector {
  ...
  Vector.polar(num len, num angle) : _x = (len * math.cos(angle)).toDouble(), _y = (len * math.sin(angle)).toDouble();
  ...
  num get degrees => math.atan2(_y, _x) * 180 / math.PI;
  num get rads => math.atan2(_y, _x);
  ...
  Vector slerp(Vector vec, num t) {
    ...
    num theta = math.acos(cosTheta);
    num sinTheta = math.sin(theta);
    
    if (sinTheta <= Epsilon)
      return vec.clone();
    
    num w1 = math.sin((1 - t) * theta) / sinTheta;
    num w2 = math.sin(t * theta) / sinTheta;
